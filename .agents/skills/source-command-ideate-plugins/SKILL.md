---
name: "source-command-ideate-plugins"
description: "Brainstorm complementary plugin ideas that would fill gaps in the current marketplace."
---

# source-command-ideate-plugins

Use this skill when the user asks to run the migrated source command `ideate-plugins`.
...
1. **Map the current marketplace.** Parse `.Codex-plugin/marketplace.json` and group the plugins by theme (AI tooling, dev workflow, sysadmin, media, writing, personal productivity, etc.). Note what Daniel clearly cares about based on what he's already shipped.
...
2. **Identify gaps and adjacencies.** For each theme cluster, ask:
   ...
   - What newer Codex features (skills, hooks, MCP integrations) could unlock a plugin type Daniel hasn't tried?
...
- Don't suggest things that are really MCP servers, standalone CLIs, or libraries rather than Codex plugins.
