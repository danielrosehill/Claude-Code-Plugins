---
name: "source-command-prune-plugins"
...
# source-command-prune-plugins

Use this skill when the user asks to run the migrated source command `prune-plugins`.
...
1. **Load the marketplace.** Parse `.Codex-plugin/marketplace.json` and list every plugin with its `name`, `description`, `tags`, and `source.repo`. Also list the `plugins/` submodule directories.
...
3. **Flag candidates for removal in these buckets:**
   ...
   - **Too niche / Daniel-specific** — plugins that slipped through depersonalisation and aren't useful to others (reference `AGENTS.md`: "Plugins Shouldn't Be Daniel-Specific")
...
6. **For each confirmed removal, perform the full cleanup:**
   ...
   ```
   Then:
   - Remove the entry from `.Codex-plugin/marketplace.json` (validate JSON after)
